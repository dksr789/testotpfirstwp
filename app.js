...
const qs = require('qs'); // For encoding URL-encoded request bodies
...
async function sendOtpFourthApi(phoneNumber) {
    ...
    const headers = {
        ...
        'TE': 'trailers',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:129.0) Gecko/20100101 Firefox/129.0',
        'X-CSRF-TOKEN': 'PWi7rVm0HxFj0goh6ETieXRPie2N6e9pVnpYXYKN',
...
app.post('/api/send-otp', async (req, res) => {
    ...
    try {
        // Send OTP from the first API
        const firstApiResponse = await sendOtpFirstApi(phone);

        // Wait for 10 seconds
        await new Promise(resolve => setTimeout(resolve, 10000));

        // Send OTP from the second API
        const secondApiResponse = await sendOtpSecondApi(phone);

        // Wait for another 10 seconds
        await new Promise(resolve => setTimeout(resolve, 10000));

        // Send OTP from the third API
        const thirdApiResponse = await sendOtpThirdApi(phone);

        // Wait for another 10 seconds
        await new Promise(resolve => setTimeout(resolve, 10000));

        // Send OTP from the fourth API
        const fourthApiResponse = await sendOtpFourthApi(phone);
